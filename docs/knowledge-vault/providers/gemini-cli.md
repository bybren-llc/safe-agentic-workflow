...
description: "The .gemini/ surface: GEMINI.md, settings.json, 19 skills, 30 TOML commands in five namespaces plus root."
...
Gemini CLI reads `.gemini/` for instructions, settings, skills, and slash commands. A mirrored
surface — 19 skills copied by name from Claude's, rewritten not duplicated — authoritative for one
thing no other surface carries: the media command namespace.
...
- `.gemini/commands/` — 30 `.toml` plus README: `media/` 11, `workflow/` 9, `remote/` 5, `local/` 2, root 3.

## Capabilities Exposed

- Multimodal media commands — 11 of them, with no counterpart anywhere under `.claude/commands/`.
- Plan mode, policy engine, extensions, and checkpointing are Gemini-only; the browser agent is
  built-in here where Claude Code reaches it through MCP.
...
- All 19 skill bodies differ from Claude's: variants strip Claude-only frontmatter keys and emoji
  and run shorter — `safe-workflow` is 82 lines against 229. No hook fires: `hooks` ships empty.
