...
closes the loop opened by [`/start-work`](start-work.md) and hands off to [`/pre-pr`](pre-pr.md).
...
- Notes that GitHub-Linear auto-closes tickets named in commit messages on merge; unreferenced child
  stories must be closed by hand.

## Provider Parity

| Surface | Behaviour |
| --- | --- |
| Claude | classifies against `origin/dev` |
...
| Both | commit block byte-identical and unexecuted; only Gemini's lead-in wording differs |
