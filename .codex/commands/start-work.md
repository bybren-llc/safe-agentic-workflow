...
1. **Linear Ticket Exists?**
   - If no ticket number provided, ask user for Linear ticket number
   - Verify ticket exists in Linear (use web UI or CLI)
...
3. **Branch Naming**
   ...
   - Must start with ticket prefix and number
   - Use lowercase with hyphens

4. **Start from Latest Main**
   - Ensure starting from clean main branch: `git checkout {{MAIN_BRANCH}} && git pull origin {{MAIN_BRANCH}}`
